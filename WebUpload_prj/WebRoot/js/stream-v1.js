...
(function(){
	...
	function Main(cfg){
		...
		this.config = {
			...
			dragAndDropTips: cfg.dragAndDropTips || "<span>鎶婃枃浠�鏂囦欢澶�鎷栨嫿鍒拌繖閲�/span>",
			fileFieldName : cfg.fileFieldName || "FileData",
			browser : cfg.browser,
			browseFileId : cfg.browseFileId || "i_select_files",
			browseFileBtn : cfg.browseFileBtn || "<div>璇烽�鎷╂枃浠�/div>",
	...
	Main.prototype = {
		...
		upload : function(index) {
			...
			window.XMLHttpRequest && (xhr.onerror = function() {
				self.cancelOne(index);
				var msg = "\u521B\u5EFA\u4E0A\u4F20\u4EFB\u52A1\u5931\u8D25,\u72B6\u6001\u7801:window.XML*****" + xhr.status + ",\u8BF7\u68C0\u6D4B\u7F51\u7EDC...";
				fShowMessage(msg, true);
			});
			xhr.send();
		},
		...
		validateFile : function(uploader) {
			...
			if(!bStreaming && size > 2147483648){this.uploadError({status:100, statusText:"Flash鏈�ぇ鍙兘涓婁紶2G鐨勬枃浠�"});return !1;}
	...
	window.Stream = window.Uploader = Main; /** window.Uploader鏄疭WF缁勪欢鐨勫叧閿瓧(淇濈暀) */
})();
