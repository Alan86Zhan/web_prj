package cn.twinkling.stream.util;

import java.io.IOException;

/**
 * Key Util: 1> according file name|size ..., generate a key;
 * 			 2> the key should be unique.
 */
public class TokenUtil {

	/**
	 * 鐢熸垚Token锛�A(hashcode>0)|B + |name鐨凥ash鍊紎 +_+size鐨勫�
	 * @param name 	
	 * @param size
	 * @return
	 * @throws Exception
	 */
	public static String generateToken(String name, String size)
			throws IOException {
		System.out.println("****TokenUtil****");
		System.out.println("****TokenUtil**name**"+name);
		if (name == null || size == null)
			return "";
		int code = name.hashCode();
		try {
			String token = (code > 0 ? "A" : "B") + Math.abs(code) + "_" + size.trim();
			System.out.println("****TokenUtil**token**"+token);
			/** TODO: store your token, here just create a file */
			IoUtil.storeToken(token);
			System.out.println("****IoUtil**token**"+token);
			return token;
		} catch (Exception e) {
			throw new IOException(e);
		}
	}
}
